    ...
    return (
        <div className=" bg-white cursor-pointer w-56 h-60 rounded-lg">
            <figure className="relative mb-2 w-full h-4/5"> 
                ...
                <img className="w-full h-full object-cover rounded-lg" src={data?.data?.image} alt={data?.data?.title} />
                <div className=" absolute top-0 right-0 flex justify-center items-center bg-white w-6 h-6 rounded-full m-2 p-1 py-0.5">+</div>
            </figure>
            <p className="flex justify-between">
                <span className="text-sm font-light truncate" title={data?.data?.title}>{data?.data?.title}</span>
